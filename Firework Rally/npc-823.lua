...
local battleBombs = require("scripts/battleBombs")
...
local fireworkExplosion = Explosion.register(823, 78, 78, fireworkSFX, true, false)
...
function firework.onTickNPC(v)
	...
	if v.speedY >= -1 then		-- if it came to a stop, then explode!
		...
		-- give coins for blocks hit
		local p = Player(data.playerIdx)
		local blockHitCollider = Colliders.Circle(v.x + v.width * 0.5, v.y + v.height * 0.5, NPC.config[v.id].explosionEffectRadius)
		for _,b in ipairs(Colliders.getColliding{a = blockHitCollider, btype = Colliders.BLOCK}) do
		    if data.playerIdx and b.contentID > 0 and b.contentID < 100 then
		        battleItems.addCoins(p,1,b.x + b.width * 0.5,b.y)
		    end
		end
		
		data.mimicID = v.id -- mimic yourself (check your own config for explosion ID and radius)
		fireworkAI.drawParticles(v.x+v.width*0.5,v.y+v.height*0.5,data.colourType,data.colour)
		battleBombs.explode(v,data.playerIdx)
		--v:kill()
	end
end
