...
function hanabihai.explodeSegment(v)
	...
	if data.segmentCount == 1 then	-- makes the last explosion and kills the npc
		...
		effect.speedY = -4
		local prevHeight = v.height
		v.height = data.baseHeight + (data.segmentCount) * 10
		v.y = v.y - (v.height - prevHeight)
	end
end
...
function hanabihai.onTickNPC(v)
	...
	if v.despawnTimer <= 0 then
	    -- weird fix to make sure despawned hanabihai keep their extra settings on respawn
	    if data.oldID then
		    local oldSettings = v.data._settings
		    v:transform(data.oldID, true, false)
		    data.hanabihaiInitialized = false
		    data.harmedHanabihaiIntialized = false
		    data._settings = oldSettings
		else
		    data.hanabihaiInitialized = false
		    data.harmedHanabihaiIntialized = false
		end
	...
	--[[	Initialize all the vars required.
		...
		if NPC.config[v.id].transformToID then  -- if this config exists, it's a walking Hanabihai
			data.baseHeight = 24
			v.height = data.baseHeight + (data.segmentCount) * 10
			v.y = v.y - (v.height - 52)
		else
