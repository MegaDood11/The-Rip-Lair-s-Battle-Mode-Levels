...
function sampleNPC.onTickNPC(v)
	...
	if not data.initialized then
		...
		data.tilt = 0
		data.timer = 0
		if v.dontMove then v.direction = RNG.irandomEntry{-1,1} end
	end
	...
	data.tilt = data.tilt + (math.sin((data.timer + 69) / 48)) * -v.direction
	
	data.rotation = data.tilt
end
