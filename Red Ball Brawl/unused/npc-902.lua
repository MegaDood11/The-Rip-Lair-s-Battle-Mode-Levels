...
function sampleNPC.onTickNPC(v)
	...
	for _,p in ipairs(Player.get()) do
		if math.abs(p.x - 16 - (v.x - data.tilt)) <= 48 and math.abs(p.y - (v.y + -math.abs(data.tilt / 2) + v.height * 0.8)) <= 48 then
			p:harm()
		end
	end
	...
	data.tilt = data.tilt + (math.sin((lunatime.tick() + 69) / 48)) * -v.direction
