...
function walker.onMessageBox(eventObj, message, plyr, npc)
	...
	if npc.data._settings.behaviour == 3 then return end
	--[[
	local npc = nil;
    ...
    if npc and npc.data.isWalker then
		npc.data.saveState = npc.data.state
...
function walker.onTickNPC(v)
	...
	if v.data.saveState then
		v.data.state = v.data.saveState
		v.data.saveState = nil
	end
    ...
    if data.state == 1 then
		if v.data._settings.behaviour == 1 then
...
function walker.onDrawNPC(v)
	...
	if v.data._settings.behaviour == 1 or v.data.state == 3 then
		local data = v.data
