...
function bud.onTickEndNPC(v)
	...
	if settings.color == 1 then --green
		v.animationFrame = math.floor(lunatime.tick() / 8) % 5 + 5
	elseif settings.color == 2 then --yellow
		v.animationFrame = math.floor(lunatime.tick() / 8) % 5 + 10
	elseif settings.color == 3 then --blue
		v.animationFrame = math.floor(lunatime.tick() / 8) % 5 + 15
	elseif settings.color == 4 then --pink
		v.animationFrame = math.floor(lunatime.tick() / 8) % 5 + 20
	else --red
		v.animationFrame = math.floor(lunatime.tick() / 8) % 5
	end
...
function bud.onNPCKill(eventObj, killedNPC, killReason)
	...
	if killReason == 1 or killReason == 2 or killReason == 3 or killReason == 4 or killReason == 7 then --if killed by these reasons
		Animation.spawn(753, v.x, v.y, settings.color+1)
	end
end
