...
function onTick()
	if not battleTimer then return end
	if battleTimer.secondsLeft == battleTimer.optionTimeValues[battleOptions.getModeRuleset().timeLimit] - 90 and thing1.value == 0 then
		triggerEvent("bowserbootup")
		thing1.value = 1
	end
	if battleTimer.secondsLeft == 60 and thing2.value == 0 then
