...
function sampleNPC.onTickNPC(v)
	...
	--Execute main AI. This is not doing anything! (I want my money back)
	...
	for _,c in ipairs(Camera.get()) do
		...
		if bx + v.width >= cx and bx <= cx + c.width and by + v.height >= cy and by <= cy + c.height then
			if (not v.isHidden) and v:mem(0x5A, FIELD_WORD) == 0 then
				Graphics.drawImageToSceneWP( tipofpolesprite, v.x + 14, v.y + 8, -24) --draw the tip over the player
			end
			break
		end
	end
