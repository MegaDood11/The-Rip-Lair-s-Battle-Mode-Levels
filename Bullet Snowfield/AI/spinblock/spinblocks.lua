...
local spinblocks = {
	spinGrav = 0.1,					-- gravity the player has while spinning; lower gravity means higher bounces!
	...
	accelerationDecrease = 0.01,	-- subtracted from the player speed when trying to accelerate; 0.1 is Mario's default acceleration in air before reaching walking speed cap, then 0.5
	...
	canCancelDrill = true,			-- if true, you stop drilling when stopping to hold down
...
spinblocks.bannedNPCs = {	-- list of all npc ids for npcs the player should be able to hold when flying. By default has both grabbable propeller blocks since they enable infinite flight
	278, 279,
}
...
local function canFly(p)
	...
	return (
		...
		Level.winState() == 0 					-- has not finished the level yet
	...
	for _,p in ipairs(Player.get()) do	-- check whether a player is on top of the thing
		if not p.data.spinblocks then
			p.data.spinblocks = {
				...
				launchSpeed = 0,
				isDrilling = false,
				drillCollider = Colliders.Box(p.x,p.y + p.height, p.width, 16),
...
function spinblocks.onDraw()
	for _,p in ipairs(Player.get()) do
		local data = p.data.spinblocks
		if data.isDrilling and data.isFlying then
