--[[
	...
	Modified by MegaDood
	Further improved by Marioman2007
...
function bully.onNPCHarm(eventObj,v,reason,culprit,w)
	...
	elseif data.state ~= STATE_SINK then
		...
		if culprit then
			if reason == HARM_TYPE_JUMP or reason == HARM_TYPE_SPINJUMP then
				data.springIdle = false
				data.state = STATE_SPRING
				data.springTimer = 0
				data.springFrame = 0
			else
...
function bully.onTickEndNPC(v)
	...
	local data = v.data
	data.boost = data._settings.boost or 0
	data.boostTurbo = data._settings.boostTurbo or 0
	...
	local plr = Player.getNearest(v.x, v.y, v.x + v.width, v.y + v.height - 8)

	if Colliders.speedCollide(plr,v) and plr.speedY > 1 and plr.y < v.y - (plr.height/2) then
		local e = Effect.spawn(75,0,0)
		e.x = plr.x - (plr.width/2) + RNG.random(-10, 10)
		e.y = plr.y + e.height/2

		if plr.keys.jump or plr.keys.altJump then
			plr.speedY = data.boostTurbo
		else
			plr.speedY = data.boost
		end
	...
	elseif data.state == STATE_SPRING then
		if not data.springIdle then
			v.animationFrame = (math.floor(data.springFrame / NPC.config[npcID].springFrSpeed) % NPC.config[npcID].springFrames) + NPC.config[npcID].wanderFrames + NPC.config[npcID].chaseFrames
		elseif data.springIdle then
			v.animationFrame = NPC.config[npcID].frames - 1 -- the last frame should be the idle spring frame
		end

		v.speedX = 0
		data.springTimer = data.springTimer + 1
		
		-- calculate the delay of the spring animation
		local sprngAnimDelay = (NPC.config[npcID].springFrSpeed * (NPC.config[npcID].springFrames - 1)) + NPC.config[npcID].springFrames

		local sprngDelay = NPC.config[npcID].springDelay

		if data.springTimer <= sprngAnimDelay then
			data.springIdle = false
			data.springFrame = data.springFrame + 1
		elseif data.springTimer > sprngAnimDelay and data.springTimer < sprngDelay then
			data.springIdle = true
		elseif data.springTimer >= sprngDelay then
			data.springIdle = false
			data.springFrame = data.springFrame - 1
		end
		
		if data.springTimer >= sprngAnimDelay + sprngDelay then
			data.springIdle = false
			data.state = STATE_WANDER
			data.springTimer = 0
			data.springFrame = 0
...
onlinePlayNPC.onlineHandlingConfig[npcID] = {
	...
	setExtraData = function(v, receivedData)
		...
		data.boost = receivedData.boost
		data.boostTurbo = receivedData.boostTurbo
