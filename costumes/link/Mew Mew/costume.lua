...
costume.goaltapeColor = Color.fromHexRGBA(0xFF8A90FF)

goaltape.registerVictoryPose("Mew Mew",28,28) -- this allows the player to use a custom frame after getting a goal tape
...
local function getCostumeFilename(filename)
	return "costumes/link/Mew Mew/"..filename
end
...
function costume.onInit(p)
	...
	Routine.run(function()
		...
		if not hasInitialized then
			hurtSFX = SFX.open(getCostumeFilename("mew-hurt.ogg"))
			deathSFX = SFX.open(getCostumeFilename("mew-death.ogg"))
