...
goaltape.registerVictoryPose("SMW-Link",28,29) -- this allows the player to use a custom frame after getting a goal tape
...
local clearPipeHorizontalFrames = table.map{2,8,42,44}
local clearPipeVerticalFrames = table.map{9,10,15}
...
local function handleDucking(p)
	...
	-- workaround for link being forced to duck upon jumping upwards, desyncing the sprites as a result
	if p.character == CHARACTER_LINK then
		local settings = PlayerSettings.get(CHARACTER_LINK, p.powerup)
		if not isOnGround(p) then
			settings.hitboxDuckHeight = settings.hitboxHeight
		else
			settings.hitboxDuckHeight = costume.playerData[p].duckedHeight
		end
	end
...
local animations = {
	...
	-- animations for link specifically
	hurt = {11},
	jump = {22},
	skid = {33},
	swipeAir = {6,7,6,frameDelay = 6,loops = false}
}
...
local function findAnimation(p)
	...
	local isSwiping = p:mem(0x14, FIELD_WORD) ~= 0
	...
	elseif (p:mem(0x34,FIELD_WORD) > 0 and p:mem(0x06,FIELD_WORD) == 0) and p.holdingNPC == nil then -- swimming
		...
		if p:mem(0x38,FIELD_WORD) == 15 then
			return "swimStroke"
		elseif ((data.currentAnimation == "swimStroke" and p.powerup ~= PLAYER_SMALL)) and not data.animationFinished then
			return data.currentAnimation
		end
		...
		if p.invincibilityTimer >= 135 then
			return "hurt"
		end
		...
		if p.speedY < Defines.player_grav and not p.keys.up and canFall(p) then 
			return "jump"
		end
...
function costume.onInit(p)
	...
	if costume.playerData[p] == nil then
		local settings = PlayerSettings.get(CHARACTER_LINK, PLAYER_BIG)
		...
		costume.playerData[p] = {
			...
			duckedHeight = settings.hitboxDuckHeight
		}
...
function costume.onTickEnd()
	for _,p in ipairs(costume.playersList) do
		if p.character == CHARACTER_LINK then
			...
			if canFall(p) then
				data.useFallingFrame = (data.useFallingFrame or p.speedY > 0 and not p.keys.down)
			else
