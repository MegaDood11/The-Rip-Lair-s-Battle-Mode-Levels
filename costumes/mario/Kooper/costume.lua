--[[
	...
	(costume settings based on SMW-Mario)
...
local duckHeight = 30
local normalHeight = 54

local acceleration = 0.17	-- how much speed the player gains by holding in a direction
local accelerationPastWalkspeed = 0.12
...
goaltape.registerVictoryPose("Kooper",28,29) -- this allows the player to use a custom frame after getting a goal tape
...
local function isSupposedToBeSmall(p)
	return (
		(
			p:mem(0x164,FIELD_WORD) ~= 0 or 	-- tail swiping
			--(p:mem(0x50,FIELD_BOOL) and (p.powerup == 4 or p.powerup == 5)) or	-- spinjumping with super leaf/tanooki
			p:mem(0x3C,FIELD_BOOL) 		-- sliding
		) and not (
			p.powerup == PLAYER_SMALL or
			p.isTanookiStatue or
			p.rainbowShellSurfing or
			p.isMega or
			p.forcedState ~= FORCEDSTATE_NONE or	-- not doing something wacky like going into a door, pipe, powering up etc
			p.isSpinJumping
		)
	)
end
...
local animations = {
	...
	lookUp = {32},
	lookUpHolding = {33},
	spinJump = {1,15,1,13 ,frameDelay = 3},
	...
	shootGround = {12,11,11, frameDelay = 4,loops = false},
	shootAir    = {12,11,11, frameDelay = 4,loops = false},
	shootWater  = {12,11,11, frameDelay = 4,loops = false},


	-- Leaf things
	slowFall = {11,5, frameDelay = 6},
	runSlowFall = {19,36,37,36, frameDelay = 5},
	
	flyLeaf = {19,36,37,36, frameDelay = 5},
	tailAttack = {1,15,-1,-13, frameDelay = 3},
...
local function findAnimation(p)
	...
	elseif p.forcedState == FORCEDSTATE_DOOR then
		...
		return nil
	elseif p.forcedState == FORCEDSTATE_POWERDOWN_SMALL or p.forcedState == FORCEDSTATE_POWERDOWN_FIRE or p.forcedState == FORCEDSTATE_POWERDOWN_ICE then
		return "damaged"
	...
	if p:mem(0x16E,FIELD_BOOL) and not p.holdingNPC then -- is flying (but not holding anything)
		return "flyLeaf"
	end
	...
	if isOnGround(p) then
		...
		if p.holdingNPC and p.powerup == PLAYER_SMALL then
			return "holdSmallIdle"
		end
		return nil
	elseif (p:mem(0x34,FIELD_WORD) > 0 and p:mem(0x06,FIELD_WORD) == 0) and p.holdingNPC == nil then -- swimming
		...
		if p.holdingNPC == nil then
			...
			elseif not isOnGround(p) and p.powerup == PLAYER_SMALL then
				return "jumpSmall"
			end
		else
			if data.useFallingFrame and p.powerup == PLAYER_SMALL then
				return "fallSmallHolding"
			elseif not isOnGround(p) and p.powerup == PLAYER_SMALL then
				return "jumpSmallHolding"
			end
		end
