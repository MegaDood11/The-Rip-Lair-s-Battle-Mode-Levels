...
goaltape.registerVictoryPose("SMB3-Luigi",28, 29) -- this allows the player to use a custom frame after getting a goal tape
...
if p.keys.down and not smb2Characters[p.character] and p.powerup == PLAYER_SMALL and not p.holdingNPC and canDuck(p) then
...
local animations = {
	...
	run  = {16,17,16,18, frameDelay = 4},
	walkHolding = {9,10,9,8, frameDelay = 4},
	jump = {4,46,47, frameDelay = 3},
	fall = {5,32,33, frameDelay = 3},
	...
	shootGround = {11,12, frameDelay = 6,loops = false},
	shootAir    = {44,43,40, frameDelay = 6,loops = false},
	shootWater  = {44,43,40, frameDelay = 6,loops = false},


	-- Leaf things
	slowFall = {5,32,39,3,36,37, frameDelay = 3},
	crouchSlowFall = {27,48,49, frameDelay = 5},
	runSlowFall = {21,20,19, frameDelay = 3},
	fallLeafUp = {4,46,47, frameDelay = 3},
	crouchFallLeaf = {49},
	runJumpLeafDown = {18},
	...
	swimStroke = {40,43,44, frameDelay = 4,loops = false},
	swimStrokeSmall = {42,43, frameDelay = 4,loops = false},
...
local function findAnimation(p)
	...
	if leafPowerups[p.powerup] and p:mem(0x12E,FIELD_BOOL) then
		if isSlowFalling(p) then
			return "crouchSlowFall"
		elseif p.speedY > 0 then
			return "crouchFallLeaf"
		end	
	end
	...
	elseif (p:mem(0x34,FIELD_WORD) > 0 and p:mem(0x06,FIELD_WORD) == 0) and p.holdingNPC == nil then -- swimming
		...
		if p.holdingNPC == nil then
			...
			elseif leafPowerups[p.powerup] and p.speedY <= 0 then
				return "fallLeafUp"
			elseif p.powerup == PLAYER_SMALL and p.speedY ~= 0 then
				return "jumpSmall"
			elseif p.speedY <= 0 and p.powerup ~= PLAYER_SMALL then
				return "jump"
			else
				return "fall"
			end
