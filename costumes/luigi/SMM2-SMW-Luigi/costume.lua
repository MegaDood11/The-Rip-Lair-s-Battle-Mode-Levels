...
local animations = {
	...
	duckSlowFall = {47,48,49, frameDelay = 5},
	duckFallLeaf = {49},
	...
	-- Luigi exclusive animations
	jump = {4,36,46,frameDelay = 3},
	jumpSmall = {3,36,frameDelay = 3},
}
...
local function findAnimation(p)
	...
	if p:mem(0x12E,FIELD_BOOL) then
		...
		elseif p.holdingNPC ~= nil then
			return "duckHolding"
		elseif leafPowerups[p.powerup] then 
	...
	elseif (p:mem(0x34,FIELD_WORD) > 0 and p:mem(0x06,FIELD_WORD) == 0) and p.holdingNPC == nil then -- swimming
		...
		if p.speedY < Defines.player_grav and canFall(p) then 
			if not p.holdingNPC then
				if p.powerup <= 1 then
					return "jumpSmall"
				else
					return "jump"
