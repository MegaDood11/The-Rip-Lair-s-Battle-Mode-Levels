...
goaltape.registerVictoryPose("SMM2-SMB3-Toadette",28, 29) -- this allows the player to use a custom frame after getting a goal tape
...
local characterMap = {"mario","luigi","peach","toad","link","megaman","wario"}
...
local animations = {
	...
	duck = {46,7, frameDelay = 5, loops = false},
	duckSmall = {46,7, frameDelay = 5, loops = false},
	...
	duckSlowFall = {36,37,38, frameDelay = 5},
	...
	swimStrokeSmall = {41,42,43, frameDelay = 4,loops = false},


	-- To fix a dumb bug with toad's spinjump while holding an item
	spinjumpSidwaysToad = {8},
	
	-- When duck-falling using the toadette costume
	duckFallToadette = {46},
}
...
local function findAnimation(p)
	...
	if p:mem(0x12E,FIELD_BOOL) then
		if leafPowerups[p.powerup] and isSlowFalling(p) then
			return "duckSlowFall"
		elseif p.speedY > 0 and p.holdingNPC == nil then
			return "duckFallToadette"
		elseif smb2Characters[p.character] and p.holdingNPC ~= nil then
			return "duckHolding"
...
function costume.onTick()
	for _,p in ipairs(costume.playersList) do
		if p.character == CHARACTER_TOAD then
			local data = costume.playerData[p]
