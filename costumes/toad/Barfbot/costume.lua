...
goaltape.registerVictoryPose("SMW-Toad",28,29) -- this allows the player to use a custom frame after getting a goal tape
...
local function handleDucking(p)
	...
	if smb2Characters[p.character] and p.holdingNPC ~= nil and p.holdingNPC.isValid and not isInClearPipe(p) then
		...
		local heldNPCX = p.x + p.width*0.5 - p.holdingNPC.width*0.5 + (settings.grabOffsetX*p.direction)
...
local animations = {
	...
	clearPipeHorizontal = {46, setFrameInOnDraw = true},
	clearPipeVertical = {15, setFrameInOnDraw = true},


	-- Fire/ice/hammer things
	shootGround = {11,12,11, frameDelay = 6,loops = false},
	shootAir    = {36,37,47, frameDelay = 4,loops = false},
	shootWater  = {36,37,47, frameDelay = 4,loops = false},


	-- Leaf things
	slowFall = {2,3,5, frameDelay = 5},
	runSlowFall = {2,3,5, frameDelay = 5},
	fallLeafUp = {11},
	runJumpLeafDown = {5},


	-- Swimming
	swimIdle = {2,1, frameDelay = 10},
	swimStroke = {4,5,2, frameDelay = 4,loops = false},
	swimStrokeSmall = {4,5,2, frameDelay = 4,loops = false},
