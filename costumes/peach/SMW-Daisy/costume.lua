...
local animations = {
	...
	duckHolding = {27},
	duckGrounded = {41,7, frameDelay = 5, loops = false},
	...
	shootAir    = {40,40,40, frameDelay = 6,loops = false},
	...
	-- Floating
	float = {46, frameDelay = 16, loops = true},
	duckFallPeach = {41},
}
...
local function findAnimation(p)
	...
	if p:mem(0x12E,FIELD_BOOL) then
		if p.holdingNPC ~= nil then
			return "duckHolding"
		elseif leafPowerups[p.powerup] and isSlowFalling(p) then
			return "duckSlowFall"
		elseif data.useFallingFrame and p:mem(0x1C,FIELD_WORD) <= 0 then
			return "duckFallPeach"
