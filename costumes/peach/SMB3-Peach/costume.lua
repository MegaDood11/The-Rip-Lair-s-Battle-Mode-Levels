...
goaltape.registerVictoryPose("SMW-Peach",28,29) -- this allows the player to use a custom frame after getting a goal tape
...
local function handleItemHolding(p) 
	if p.character ~= CHARACTER_PEACH or p.holdingNPC == nil or p:mem(0x12E,FIELD_BOOL) or costume.playerData[p] == nil then return end
	p.holdingNPC.y = p.y - p.holdingNPC.height + 8
end
...
local function handleDucking(p)
	...
	if smb2Characters[p.character] and p.holdingNPC ~= nil and p.holdingNPC.isValid and not isInClearPipe(p) then
		-- Change the held NPC's position for peach
		local settings = PlayerSettings.get(playerManager.getBaseID(p.character),p.powerup)
...
local animations = {
	-- Big only animations
	walk = {3,2,1,2, frameDelay = 3},
	run  = {18,17,16, frameDelay = 4},
	walkHolding = {10,9,8,9, frameDelay = 3},
	fall = {5},
	duckSmall = {8},
	...
	-- Leaf things
	slowFall = {19,20,21, frameDelay = 5},
	...
	-- Peach exclusive frames
	-- Floating
	float = {45, frameDelay = 16, loops = false},
}
...
local function findAnimation(p)
	...
	elseif (p:mem(0x34,FIELD_WORD) > 0 and p:mem(0x06,FIELD_WORD) == 0) and p.holdingNPC == nil then -- swimming
		...
		if p:mem(0x1C,FIELD_WORD) > 0 and p.holdingNPC == nil then -- floating as peach
			return "float"
		end
